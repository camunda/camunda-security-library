...
public final class OidcRedirectionEndpoint {
   ...
   *
   * <p>The context path is stripped only from a value that spells it out itself, such as the
   * absolute {@code https://host.example.com/orchestration/sso-callback} the Camunda chart renders
   * for a context-path'd webapp. A value that carries it through {@code {baseUrl}} or {@code
   * {basePath}} instead has it accounted for already — the resolver expands both from {@code
   * request.getContextPath()} — so what follows the placeholder is the context-relative path, and
   * stripping again would mount the endpoint one segment above the callback the IdP redirects to.
   *
   * <p>A {@code {registrationId}} placeholder (as in Spring's default template {@code
   * {baseUrl}/login/oauth2/code/{registrationId}}) is rewritten to a {@code *} wildcard, so the
   * matcher states the intent — any registration id — rather than relying on the {@code
   * PathPatternRequestMatcher} behind {@code redirectionEndpoint().baseUri(...)} reading the
   * left-over placeholder as a single-segment path variable, which is what makes an unexpanded
   * placeholder match a concrete callback such as {@code /login/oauth2/code/oidc}.
   *
