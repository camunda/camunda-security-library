 ...
 * ScopedSecurityChainRegistrar} for physical-tenant scopes. See ADR-0017 for why filters are
...
public class WebSessionConfiguration {
   ...
   * to its scope (ADR-0017). The {@link ScopedSessionStorePortProvider} is optional — when a host
   ...
   * per-scope one (ADR-0017). Resolved per sweep so scoped repositories created after startup are
   * included.
   */
