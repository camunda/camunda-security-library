 ...
 *
 * <p>Used to back the default {@code JwtDecoder} when {@code
 * camunda.security.authentication.oidc.additional-jwk-set-uris} is non-empty — the primary {@code
 * jwk-set-uri} is queried first, then each additional URI in declared order. The first source that
 * resolves the token's signing key wins.
 *
 * <p>Thread-safe: the source list is immutable and each delegate {@link JWKSource} is responsible
 * for its own internal thread safety.
 *
