...
public final class ScopedClientRegistrationFactory {
   ...
   *     absolute http(s) URL with a usable host and port. The URL checks run before the
  ...
  private static void requireAbsoluteHttpUrl(
    ...
    if (!parsed.isAbsolute()
        ...
        || !namesAPortASocketCanUse(parsed)) {
  ...
  private static String endpointUrlError(
    ...
    return "Cannot build ClientRegistration '"
        ...
        + "': "
        + property
        + " must be an absolute http(s) URL with a usable host and port, but was: "
        + value
    ...
    if (StringUtils.hasText(oidc.getRedirectUri())) {
      ...
      if (checkCallback && !isUsableRedirectUri(configured, registrationId)) {
        throw new IllegalArgumentException(
            "Cannot build ClientRegistration '"
                + registrationId
                + "': redirect-uri must expand to an absolute http(s) URL with a usable host and"
                + " port and a callback path, and without a fragment, because that is where the IdP redirects"
   ...
   * variable names — to an absolute http(s) URL with a {@link #namesAPortASocketCanUse usable host
   * and port} and a path, carry no fragment (RFC 6749, section 3.1.2, forbids one; a query is
   * allowed and is passed on to the IdP) and a path {@link #isServableByTheDefaultFirewall the
   * default firewall lets through}, and name a callback {@link
  ...
  /**
   * Whether the URL's authority names a port a socket can be opened on. {@link URI} and {@link
   * java.net.URL} take any run of digits, so an out-of-range port such as {@code :99999} is only
   * caught when the connection is attempted; {@link InetSocketAddress#createUnresolved} applies the
   * range rule without resolving the host, so spotting it needs no network access. A {@code -1}
   * port means the URL names none, which is the common case.
   */
  private static boolean namesAPortASocketCanUse(final URI url) {
    if (url.getPort() == -1) {
      return true;
    }
    try {
      InetSocketAddress.createUnresolved(url.getHost(), url.getPort());
      return true;
    } catch (final IllegalArgumentException portOutOfRange) {
      return false;
    }
  }
  ...
  private static boolean isUsableForRequestShape(
    ...
    return expanded.isAbsolute()
        ...
        && namesAPortASocketCanUse(expanded)
