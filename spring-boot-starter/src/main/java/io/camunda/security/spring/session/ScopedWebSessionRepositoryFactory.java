 ...
 * every per-scope store via {@link #builtRepositories()} (ADR-0012) — each store is swept by its
...
public final class ScopedWebSessionRepositoryFactory {
   ...
   * the expiry sweep to clean every store in turn (ADR-0012). By the time the first sweep runs, all
