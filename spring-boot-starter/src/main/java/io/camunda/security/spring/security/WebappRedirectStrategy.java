...
public final class WebappRedirectStrategy implements RedirectStrategy {
  ...
  public void sendRedirect(
    ...
    if (url == null || DEFAULT_REDIRECT_URL.equals(url)) {
      ...
      if (message != null) {
        // Strip CR/LF characters to prevent response-splitting via the header value.
        response.setHeader("X-Logout-Message", message.replaceAll("[\r\n]", ""));
      }
      return;
    }
    ...
    objectMapper.writeValue(writer, new RedirectResponse(url));
    writer.flush();
  }
