...
/**
 * Filter that enforces per-web-app authorization for webapp requests. The host plugs in:
 *
 * <ul>
 *   <li>{@link WebAppProvider} — maps a request to a web-app name (Hub: constant, OC: derived from
 *       the URL path).
 *   <li>{@link AuthorizationPort} — decides whether the principal has {@code ACCESS} on the
 *       resolved web app.
 *   <li>{@link WebAppAccessDeniedHandler} — invoked when access is denied (default: redirect to
 *       {@code <contextPath>/<webApp>/forbidden}; hosts can return 403 JSON, etc.).
 ...
 * WebAppProvider#webAppFor(HttpServletRequest)} returns empty.
 */
public final class WebAppAuthorizationCheckFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    final ResourceAccess access =
        authorizationPort.lookup(authentication, Authorization.webAppAccess(resolved));
    ...
    LOG.warn("Access denied for web app '{}' at {}", resolved, request.getRequestURI());
