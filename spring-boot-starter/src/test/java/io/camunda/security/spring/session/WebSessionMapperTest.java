...
public class WebSessionMapperTest {
  ...
  void toPersistentSessionWithCamundaAuthentication() {
    ...
    // when
    final var persistentSession = webSessionMapper.toPersistentSession(webSession);

    assertThat(persistentSession).isNotNull();
    assertThat(persistentSession.id()).isEqualTo("sessionId");
    assertThat(persistentSession.attributes()).hasSize(1);
    assertThat(persistentSession.attributes().get(CAMUNDA_AUTHENTICATION_SESSION_HOLDER_KEY))
  ...
  void toPersistentSession() {
    ...
    assertThat(persistentSession).isNotNull();
    assertThat(persistentSession.id()).isEqualTo("sessionId");
    assertThat(persistentSession.creationTime()).isEqualTo(now.toEpochMilli());
    assertThat(persistentSession.lastAccessedTime()).isEqualTo(now.toEpochMilli());
    assertThat(persistentSession.maxInactiveIntervalInSeconds())
        .isEqualTo(maxInactiveInterval.getSeconds());
    assertThat(persistentSession.attributes()).hasSize(2);
    assertThat(persistentSession.attributes().get("attribute1"))
        .isEqualTo(webSessionAttributeConverter.serialize("value1"));
    assertThat(persistentSession.attributes().get("attribute2"))
  ...
  void fromPersistentSession() {
    ...
    // when
    final var webSession = webSessionMapper.fromPersistentSession(persistentSession);
