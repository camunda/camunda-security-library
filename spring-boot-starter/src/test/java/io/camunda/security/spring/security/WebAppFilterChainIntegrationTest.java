...
/**
 * Verifies that {@link WebAppAuthorizationCheckFilter} ends up in the OIDC and Basic-auth webapp
 * filter chains when (and only when) a host registers the required SPIs alongside the explicit
 * {@code @Import(WebAppAuthorizationFilterConfiguration.class)}.
 */
class WebAppFilterChainIntegrationTest {
  ...
  @Test
  void basicChainOmitsWebAppAuthorizationFilterWhenSpisAreAbsent() {
    basicRunner.run(
        ctx -> {
          assertThat(ctx).doesNotHaveBean(WebAppAuthorizationCheckFilter.class);
          assertThat(filtersOf(ctx.getBean(BASIC_CHAIN_BEAN, SecurityFilterChain.class)))
              .noneSatisfy(f -> assertThat(f).isInstanceOf(WebAppAuthorizationCheckFilter.class));
        });
  }
  ...
  @Test
  void oidcChainOmitsWebAppAuthorizationFilterWhenSpisAreAbsent() {
    oidcRunner.run(
        ctx -> {
          assertThat(ctx).doesNotHaveBean(WebAppAuthorizationCheckFilter.class);
          assertThat(filtersOf(ctx.getBean(OIDC_CHAIN_BEAN, SecurityFilterChain.class)))
