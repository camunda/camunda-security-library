 ...
 *       cookie value through the serializer (unit-level) and inspecting the filter's presence on
 *       the chain (structural).
 *   <li><b>Cross-scope session rejection (AC 2/4 core):</b> a session seeded directly into scope
 *       A's {@link MapSessionRepository} is found by scope A's chain (because its filter resolves
 *       the cookie named {@code camunda-session-physical-tenants-a}) but is invisible to scope B's
 *       chain (because B's filter looks for a differently-named cookie and finds nothing). The
 *       consequence is tested server-side: a protected request to scope B carrying only scope A's
 *       session cookie produces a 302 redirect to scope B's login — not an authenticated response.
 *   <li><b>Per-scope picker (AC 3):</b> {@code GET /physical-tenants/a/login} renders a 200 whose
 *       body contains links under {@code /physical-tenants/a/oauth2/authorization/…} but not under
...
class ScopedWebappSessionIsolationTest {
  ...
  void perScopeCookieNamesAreDistinct() {
    ...
    // then — cookie names must be distinct and have the correct Path
    ...
    // The two cookie names must differ
    assertThat(COOKIE_A)
        .as("scope A and scope B cookie names must be distinct")
        .isNotEqualTo(COOKIE_B);
  }
   ...
   * session cookie to a request dispatched through scope B's webapp chain.
   *
   * <p>Scope B's {@link SessionRepositoryFilter} resolves session IDs by looking up the cookie
   * named {@code camunda-session-physical-tenants-b}. The incoming request carries only a {@code
   * camunda-session-physical-tenants-a} cookie, so B's resolver finds no session ID → B treats the
   * request as unauthenticated → the security chain issues a 302 redirect to B's login.
   *
   * <p>This proves that a session minted for scope A cannot be used to access scope B, even though
   * both sessions live on the same JVM. The isolation mechanism is structural: distinct cookie
   * names (not secret values or ACL checks).
   */
  @Test
    ...
    runner()
        .run(
            ctx -> {
              ...
              // -- Locate scope A's and scope B's webapp chains and their session repositories --
              final var chainAWrapper = webappChain(ctx, "a");
              final var chainBWrapper = webappChain(ctx, "b");

              // Locate the per-scope SessionRepositoryFilter on each chain to get the backing
              // MapSessionRepository. The registrar wires distinct MapSessionRepository instances
              // per scope (D2 fallback), so we can seed A's store independently.
              // Use the wrapper directly — it delegates getFilters() to the inner chain
              ...
              seedSession.setAttribute("SPRING_SECURITY_CONTEXT", securityContext);
              ...
              // Cookie carries scope A's session cookie name (wrong name for scope B)
              ...
              // The 302 status proves scope B did not authenticate the request.
              // (The Location header contains the OIDC redirect target for scope B's single-IdP
              // setup; the exact URL is not asserted here as it depends on entry-point resolution.)
            });
  }

  /**
   * Inverse: scope B's session cookie is not honoured by scope A's chain. Demonstrates the
        ...
        .run(
            ctx -> {
              ...
              seedSession.setAttribute(
                  "SPRING_SECURITY_CONTEXT", new SecurityContextImpl(principal));
  ...
  /**
   * Sanity check: confirms (a) the seeding mechanism used in the cross-scope rejection tests above
   * is not vacuous — the session actually lands in the per-scope {@link MapSessionRepository}, (b)
   * the per-scope stores are distinct instances (D2: separate {@link MapSessionRepository} per
   * scope), and (c) a session in scope A's store is NOT findable by scope B's store (store-level
   * isolation on top of cookie-name isolation).
   *
   * <p>This provides a definitive positive control: proving the session is truly stored and that
   * the two rejection tests above cannot be vacuous false positives caused by a broken seed.
   */
  @Test
    ...
    runner()
        .run(
            ctx -> {
              ...
              // Repos must be distinct instances (D2: separate MapSessionRepository per scope)
              assertThat(repoA)
              ...
              assertThat(foundInA)
                  .as(
                      "session seeded into scope A's store must be findable by scope A's repository"
                          + " (confirms the seeding mechanism works)")
                  .isNotNull();
              ...
              assertThat(foundInB)
                  .as(
                      "session seeded into scope A's store must NOT be findable by scope B's"
                          + " repository — the two stores are independent instances, providing"
                          + " store-level isolation on top of cookie-name isolation")
                  .isNull();
            });
  }
    ...
    runner()
        .run(
            ctx -> {
              ...
              // Multi-IdP (single provider per scope here): picker renders 200
              assertThat(response.getStatus())
  ...
  /**
   * Extracts the {@link SessionRepositoryFilter} from the given {@link SecurityFilterChain}. Works
   * with both {@link org.springframework.security.web.DefaultSecurityFilterChain} and {@link
   * OrderedSecurityFilterChainWrapper} (which delegates {@code getFilters()} to the inner chain).
   */
  ...
  /**
   * Extracts the backing {@link MapSessionRepository} from the given {@link
   * SessionRepositoryFilter} via reflection. This is a deliberate white-box test: proving that the
   * repository used to seed and look up sessions is the same instance the filter was wired with.
   */
  @SuppressWarnings("unchecked")
