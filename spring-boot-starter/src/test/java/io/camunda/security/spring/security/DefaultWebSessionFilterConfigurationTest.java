...
/**
 * Verifies the {@code defaultSessionRepositoryFilter} bean's repository resolution (ADR-0017):
          ...
          assertThat(registration.isEnabled())
              .as(
                  "must be disabled — otherwise Spring Boot auto-registers the filter as a"
                      + " container-wide filter, reintroducing ADR-0017's nested-filter bug")
              .isFalse();
        });
  }
