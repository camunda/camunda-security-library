...
/**
 * The authorization a caller is required to have for an operation to be allowed.
 *
 * <p>A {@code RequiredAuthorization<T>} pairs an {@link AuthorizationResourceType} with a {@link
 * PermissionType}, scopes the requirement to one or more resource IDs (statically via {@link
 * #resourceIds()}, or per-document via {@link #resourceIdSupplier()}), optionally constrains it to
 * documents matching a {@link #condition()}, and flags whether the requirement propagates
 * transitively across resource hierarchies.
 *
 * <p>This record only <em>describes</em> what is required; evaluating the requirement against a
 * principal's granted authorizations (CSL's {@link
 * io.camunda.security.api.model.authz.Authorization}) is the checker's job. In OC, {@code
 * AuthorizationChecker} wraps one or more specs in an {@code AuthorizationCondition}, gates them
 * via an {@code AuthorizationCheck}, and resolves them against the runtime context.
 *
 * <p>The pair {@link io.camunda.security.api.model.authz.Authorization Authorization} (granted) ↔
 * {@code RequiredAuthorization} (required) is the project-wide vocabulary for the two sides of any
 ...
 * io.camunda.security.api.model.authz.Authorization}. See ADR-0014 for the naming rationale.
 *
 * <h2>Serialization</h2>
 *
 * <p>The record carries no Jackson annotations — CSL's domain is Jackson-free per ADR-0014 and the
 ...
 * non-serializable {@link #resourceIdSupplier()} and {@link #condition()} as ignored. The mixin
 * lives in OC; hosts that need a different JSON shape register their own mixin on their own {@code
 * ObjectMapper}.
 *
 * @param <T> the runtime document type the check evaluates against (used by the optional {@link
 *     #resourceIdSupplier()} and {@link #condition()})
 */
public record RequiredAuthorization<T>(
  ...
  public static <T> RequiredAuthorization<T> withRequiredAuthorization(
      final RequiredAuthorization<T> authorization, final String resourceId) {
    return authorization.withResourceId(resourceId);
  }

  public static <T> RequiredAuthorization<T> withRequiredAuthorization(
      final RequiredAuthorization<T> authorization, final Function<T, String> resourceIdSupplier) {
    return authorization.withResourceIdSupplier(resourceIdSupplier);
  }

  public RequiredAuthorization<T> withResourceId(final String resourceId) {
    return withResourceIds(List.of(resourceId));
  }

  public RequiredAuthorization<T> withResourceIds(final List<String> resourceIds) {
  ...
  public RequiredAuthorization<T> withResourceIdSupplier(
  ...
  public RequiredAuthorization<T> withResourcePropertyNames(
      final Set<String> resourcePropertyNames) {
    return new RequiredAuthorization<>(
        resourceType(),
        permissionType(),
        resourceIds(),
  ...
  public RequiredAuthorization<T> withCondition(final Predicate<T> condition) {
    return new RequiredAuthorization<>(
        resourceType(),
        permissionType(),
        resourceIds(),
  ...
  public static <T> RequiredAuthorization<T> of(
      final Function<Builder<T>, Builder<T>> builderFunction) {
  ...
  public static final class Builder<T> {
    ...
    public RequiredAuthorization<T> build() {
      return new RequiredAuthorization<>(
          resourceType,
          permissionType,
          resourceIds,
