...
- `ScopedWebappSecurityChainBuilder`: from a descriptor, build the webapp chain on `pathPort.webappPaths()` prefixed with `basePath` —
  ...
  - BASIC: form-login at `<basePath>/login` (204 + CSRF header on success, mirroring CSL's primary `BasicAuthWebappSecurityConfiguration` — not `httpBasic`, which is the API chain's mechanism) backed by the existing `BasicAuthUserDetailsPort` (ADR-0021), whose adapter resolves the PT from request context.
...
**Cookie-name derivation.** The session cookie name is `camunda-session-` + **sanitize(`basePath`)** — CSL's existing, unit-tested `ScopedApiChainRegistrar.sanitizeBasePath`: strip the leading `/`, collapse each run of non-alphanumeric characters (`[^A-Za-z0-9]+`) to a single `-`, and trim leading/trailing `-` (e.g. `/physical-tenants/tenanta` → `camunda-session-physical-tenants-tenanta`; `/api/` → `camunda-session-api`). A distinct *name* — not just a distinct `Path` — is required because the primary unprefixed `camunda-session` at `Path = /` (kept for the `default`/cluster surface) would otherwise be sent alongside the scoped cookie on nested paths, leaving two same-named cookies with undefined resolution. The registrar's existing duplicate-`basePath` rejection is extended to also reject any two scopes whose sanitized cookie names collide **and to reject an empty sanitized suffix** (a `basePath` with no alphanumerics sanitizes to `""`, which would yield the non-distinct `camunda-session-`), keeping the mapping injective without an opaque hash suffix. (Cookie names have no RFC 6265 length cap; only the ~4096-byte per-cookie budget applies, which the short session-id value never approaches — and the same startup check also caps the derived name length.)
...
1. **Session isolation lives in CSL, derived from `basePath`.** A per-scope `SessionRepositoryFilter` + cookie serializer is built once per descriptor and shared by the scope's webapp and API chains. *Alternative — rely on the servlet `HttpSession`*: rejected, the container's `JSESSIONID` can't be `Path`/name-scoped per chain. *Alternative — OC builds and contributes the session components*: rejected, it would change the contract and re-introduce host-side chain assembly (the drift risk ADR-0025 closed). Per-scope cookie isolation needs Spring Session regardless of durability, so when `camunda.security.session.persistent.enabled=false` the per-scope filter uses a **per-scope in-memory `MapSessionRepository`** (store-level per-PT isolation via separate instances); when enabled, the `SessionStorePort`-backed `WebSessionRepository`. No in-memory `SessionStorePort` impl is added — ADR-0017 stands.
