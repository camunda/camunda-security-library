...
- **Spring's `JwtIssuerAuthenticationManagerResolver`** — routes by `iss` claim at the **filter** level, returning a different `AuthenticationManager` (and therefore a different `JwtDecoder`) per request. Wrong shape for CSL: the library exposes a single `@Bean JwtDecoder` injected throughout the resource-server chain. Also, this is `iss`-routed, not key-rotation-within-one-issuer. Considered for #221 (issuer-aware decoder) where it is the correct primitive for that scope, and explicitly rejected there too because the injection shape mismatches the library contract.
...
- **Use `JwtIssuerAuthenticationManagerResolver` instead.** Rejected — wrong scope and wrong injection shape (see "Why not a Spring or Nimbus built-in" above). It is the right tool for per-issuer routing (#221), not for per-issuer multi-JWKS.
