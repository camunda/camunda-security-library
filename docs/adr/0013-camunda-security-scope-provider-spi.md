...
# ADR-0013: `CamundaSecurityScopeProvider` SPI for host-contributed path-scoped API and webapp chains
...
[ADR-0003](0003-no-spring-boot-auto-configuration.md) for how a host activates them). Those chains
...
([ADR-0009](0009-session-store-port-and-web-session-ownership.md)).
...
- **`ScopedJwtDecoderFactory`** — builds a `JwtDecoder` from an `AuthenticationConfiguration` by
  ...
  decoder of [ADR-0006](0006-multi-idp-oidc-configuration.md). A scope-specific
...
- **`ScopedWebappSecurityChainBuilder`** — the webapp sibling, symmetric to the API builder and the
  ...
  - **OIDC** — `oauth2Login` with the scope's `ClientRegistration`s (from the descriptor's merged
    ...
    IdP/login) retained per [ADR-0011](0011-oidc-bearer-tokens-on-api-chain-only.md);
  - **BASIC** — form login at `basePath + /login` backed by the host's `BasicAuthUserDetailsPort`
    ([ADR-0010](0010-user-details-port.md)), whose adapter resolves the scope from request context.
...
rules are [ADR-0009](0009-session-store-port-and-web-session-ownership.md)'s.
...
the derived-cookie-name checks described in ADR-0009.
...
[ADR-0003](0003-no-spring-boot-auto-configuration.md)), alongside the two builder configurations. No
...
- Aligns with [ADR-0003](0003-no-spring-boot-auto-configuration.md): the collector activates only
  through the umbrella or an explicit `@Import` — nothing activates from the Maven dependency alone.
- Reuses the issuer-aware decoder of [ADR-0006](0006-multi-idp-oidc-configuration.md) for per-scope
