...
- `lockAndRefresh` no longer synchronizes on the `HttpSession`. Instead it calls
  ...
  it re-evaluates `isRefreshRequired` against `lastClaimed` (the JVM-local claim), not against the
  (possibly stale) `session.getAttribute(LAST_REFRESH_ATTR)`. Only the caller whose `compute` sees a
  claim that is still expired performs the refresh (`removeCamundaAuthenticationInSession` +
