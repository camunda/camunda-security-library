...
# ADR-0017: Own the web-session lifecycle behind `SessionStorePort`, with one session filter per surface and one OIDC logout handler per chain
...
1. **Multiple webapp surfaces in one browser.** [ADR-0025](0025-camunda-security-scope-provider-spi.md)
...
the umbrella" convention, see [ADR-0008](0008-no-spring-boot-auto-configuration.md)): the host must
...
duplicate-`basePath` rejection ([ADR-0025](0025-camunda-security-scope-provider-spi.md)) is extended
...
Isolation is therefore **structural**, the webapp analogue of ADR-0025's per-scope decoder: because
...
The scoped API chain ([ADR-0025](0025-camunda-security-scope-provider-spi.md)) installs the **same**
...
client). This extends [ADR-0023](0023-oidc-bearer-tokens-on-api-chain-only.md)'s "the session is
...
- The scoped API chain reading a session means it is no longer purely stateless; intended
  ([ADR-0023](0023-oidc-bearer-tokens-on-api-chain-only.md)), but a behavioural addition relative to
  the API-only scoped chains of ADR-0025. Honouring the session there requires the API surface to be
