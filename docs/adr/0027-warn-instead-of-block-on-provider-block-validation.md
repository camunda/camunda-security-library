...
Where a caller uses the value being validated for something else — `ScopedWebappSecurityChainBuilder` composing the redirect-uri and post-logout-redirect-uri it hands to Spring — that composition also falls back to its own default instead of passing an unusable value on, so a value CSL warns about degrades to the default behaviour rather than a runtime failure elsewhere (a login loop, a 500 at logout).
