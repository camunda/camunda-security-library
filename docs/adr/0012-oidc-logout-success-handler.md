...
- **Same-origin validation inlined, not a shared port.** The redirect check has no host-overridable behaviour. Promoting it to an SPI would invite host divergence on a security-critical check. The check compares scheme, host (both case-insensitive), and effective port (default ports normalised) on parsed `URI` values rather than using a `startsWith` prefix match — prefix matching is vulnerable to host-confusion bypasses such as `https://app.example.com.evil.com/` and `https://app.example.com@evil.com/`. CR/LF-injection, relative URLs, and unparseable URIs are also rejected.
- **Auth-context validation runs before the end-session diagnostic.** Spring's `super.determineTargetUrl` returns `getDefaultTargetUrl()` for *any* non-OIDC authentication context — non-`OAuth2AuthenticationToken`, non-`OidcUser` principal, unknown `registrationId` — not only when the IdP fails to publish `end_session_endpoint`. The handler validates the auth context first and only sets `REDIRECT_MESSAGE_ATTRIBUTE` once a valid OIDC session is confirmed, so the "end_session_endpoint not available" diagnostic never surfaces for callers that were never going to support RP-initiated logout.
