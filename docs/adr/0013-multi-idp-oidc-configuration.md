...
# ADR-0013: Additive multi-IdP OIDC configuration with an issuer-aware `JwtDecoder`
...
CSL owns the OIDC webapp and API filter chains ([ADR-0008](0008-no-spring-boot-auto-configuration.md)), so it also owns the `ClientRegistrationRepository` those chains route logins through and the resource-server `JwtDecoder` they validate bearer tokens with. Spring Security's `OAuth2LoginAuthenticationFilter` routes login attempts per provider via `/oauth2/authorization/{registrationId}` URLs, so multi-IdP *login* works out of the box once multiple `ClientRegistration` instances exist. Multi-IdP *token validation* does not — that is a decoder concern.
...
Consolidates records previously numbered 0015, 0020 (see git history).
