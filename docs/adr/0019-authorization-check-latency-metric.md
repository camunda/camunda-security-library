...
# ADR-0019: `core`-owned assembly factories and native latency instrumentation for `AuthorizationCheckPort`
...
[ADR-0014](0014-unified-authz-framework-in-core.md) put the authorization graph in `core` and gave
...
- **An eager `Map`, not a provider SPI** (unlike the per-scope session-store contract in
  [ADR-0009](0009-session-store-port-and-web-session-ownership.md) §4, where the host is called back
...
- **The scoped factory takes the host's own resolver rather than building one** — this is why
  decision 3 is not the shape [ADR-0014](0014-unified-authz-framework-in-core.md) rejected under
