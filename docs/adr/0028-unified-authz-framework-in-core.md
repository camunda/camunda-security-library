...
# ADR-0028: Unify authorization checking in CSL `core` behind `AuthorizationCheckPort`
...
lazy membership resolution of [ADR-0011](0011-lazy-load-authentication-memberships.md).
...
`CamundaSecurityAutoConfiguration` umbrella ([ADR-0008](0008-no-spring-boot-auto-configuration.md)).
...
(ADR-0008); the chain configurations inject it via `ObjectProvider`, so a host that has not
