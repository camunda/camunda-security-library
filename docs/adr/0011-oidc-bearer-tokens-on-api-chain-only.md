...
# ADR-0011: OIDC bearer-token validation lives on the API chain only
...
This refines, and does not contradict, [ADR-0003](0003-no-spring-boot-auto-configuration.md), which centralised the chains but did not pin which chain owns bearer-token validation.
