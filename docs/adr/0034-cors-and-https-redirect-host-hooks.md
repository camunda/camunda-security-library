...
# ADR-0034: Host customization hooks for security filter chains
...
CSL centralises filter chain construction ([ADR-0008](0008-no-spring-boot-auto-configuration.md)),
...
- `HttpsRedirectCustomizer` — a host registers a bean to insert an HTTP→HTTPS redirect filter into
  ...
  `OidcResourceServerCustomizer` pattern (see [ADR-0008](0008-no-spring-boot-auto-configuration.md))
