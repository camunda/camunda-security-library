...
- **`CamundaWebappAttributes`** (`io.camunda.security.spring.security`) — public constants class exposing `POST_LOGOUT_REDIRECT_ATTRIBUTE` (session) and `REDIRECT_MESSAGE_ATTRIBUTE` (request). Hosts that render a post-logout page reference these constants instead of hard-coding the strings.
...
- **Same-origin validation inlined, not a shared port.** The redirect check is six lines and has no host-overridable behaviour. Promoting it to an SPI would invite host divergence on a security-critical check.
- **Constants on a new `CamundaWebappAttributes` class, not on `CamundaSecurityFilterChainConstants`.** The existing constants class is package-scoped properties and filter-chain wiring constants. Attribute names exposed to host code are a different audience — webapp page authors — and warrant a separate public surface that can grow as more webapp-side attributes accrue.
...
| Post-logout redirect attribute name | `CamundaWebappAttributes.POST_LOGOUT_REDIRECT_ATTRIBUTE` | Host reads the constant; if a host registers its own handler, the host owns the attribute name |
...
- The `CamundaWebappAttributes.POST_LOGOUT_REDIRECT_ATTRIBUTE` constant gives hosts a stable contract for reading the redirect URI from the session, decoupled from the handler's internal attribute name.
