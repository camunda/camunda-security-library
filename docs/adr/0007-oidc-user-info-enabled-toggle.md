...
- **Every configured provider for a given scope has the fetch disabled, augmentation enabled for that scope.** No registration yields an issuer+userInfoUri pair, so the issuer→userInfoUri map used to build the claims provider is empty, and `CachingOidcClaimsProvider.forConfiguredMappings` **fails fast with an `IllegalStateException`** at provider-construction time — a Spring context startup failure for the cluster-wide default, or a scope-construction failure for a per-scope config. This is a deliberate config-mismatch guard: augmentation enabled with nothing to ever augment must surface loudly, not run silently doing nothing. (A scope with *zero* OIDC providers configured at all fails earlier and separately — `OidcWebappClientBeansConfiguration`'s `ClientRegistrationRepository` bean itself refuses to build with no provider configured — so this specific guard is reached only once at least one provider exists but all of them have `userInfoEnabled=false`.)
