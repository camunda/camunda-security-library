...
- **Every configured provider for a given scope has the fetch disabled, augmentation enabled for that scope.** No registration yields an issuer+userInfoUri pair, so the issuer→userInfoUri map used to build the claims provider is empty, and `CachingOidcClaimsProvider.forConfiguredMappings` **fails with an `IllegalStateException`**. The claims provider builds that map at the first claims lookup, and not while the application starts, so the failure reaches the first request that needs augmented claims. It reaches every later such request as well, because a failed build is not cached. This is a deliberate config-mismatch guard: augmentation enabled with nothing to ever augment must surface loudly, not run silently doing nothing. (A scope with *zero* OIDC providers configured at all fails earlier and separately — `OidcWebappClientBeansConfiguration`'s `ClientRegistrationRepository` bean itself refuses to build with no provider configured — so this specific guard is reached only once at least one provider exists but all of them have `userInfoEnabled=false`.)
...
- **Per-provider by construction, no global override, for both mechanisms.** Both fields live on `OidcConfiguration`, so the additive multi-IdP shape from [ADR-0006](0006-multi-idp-oidc-configuration.md) expresses per-provider toggles for free; a separate global override would add ambiguity without behavioural gain.
...
- **Fail on an augmentation config mismatch, fail open on a request-time fetch failure.** These are different failure classes: an augmentation config that can never augment anything is a deployment mistake the operator needs to see immediately, so it fails the first request that needs augmented claims and every later one, while a transient `/userinfo` outage is an expected operational condition the auth chain must tolerate. Conflating them — either failing open on both, or failing the request on both — would hide the mistake in the first case or take down authentication in the second.
...
- Both mechanisms are per-provider with no new code paths, falling out of the additive [ADR-0006](0006-multi-idp-oidc-configuration.md) shape and the already-existing `ScopedOidcClaimsProviderFactory` — a multi-IdP, multi-scope deployment can mix fetch-enabled/disabled and augmentation-enabled/disabled per provider today.
...
- The coupling between the two mechanisms is a stated, tested behaviour (a config mismatch fails each request that needs augmented claims in one shape, and stays a silent per-issuer no-op in the other) rather than an undocumented trap a host discovers via a support ticket.
...
- **On an augmentation config mismatch (augmentation enabled, no provider yields a URI), fail open instead of failing the request.** Rejected — an operator who enables augmentation and gets no error, yet also gets no augmented claims, has no signal anything is wrong. A loud `IllegalStateException` on each request that needs augmented claims is cheaper to diagnose than a silently-inert feature discovered much later.
