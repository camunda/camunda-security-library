...
- Embedded into Hub, Orchestration Cluster, and Optimize.
...
- One identity plane for Hub, OC, and Optimize, with Hub as policy SoT whenever present.
...
4. **Observability and logging**
   ...
   - Policy propagation (Hub → OC/Optimize → Engine) is designed to be observable per cluster with clear status (last version applied, last error, latency) and logs for both success and failure paths — the propagation path itself is not yet implemented, see [rollout status](./02-current-state.md#21-rollout-status-at-a-glance).
