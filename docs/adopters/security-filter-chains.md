...
| `redirect-uri` | string | unset (effective `{baseUrl}/sso-callback`) | OAuth2 `redirect_uri` that CSL sends to the IdP, and the path where the webapp chain mounts its redirection endpoint. CSL rejects the value at startup if it cannot be both. See [`redirect-uri` and the redirection endpoint](#redirect-uri-and-the-redirection-endpoint). |
...
One value decides two things: the `redirect_uri` that CSL sends to the IdP, and the path where the webapp chain mounts its redirection endpoint. A host can therefore point the callback at a path that its IdP client already has (see [ADR-0018](../adr/0018-optimize-reuses-stateful-oidc-webapp-chain.md)). Spring expands only `{baseUrl}`, `{baseScheme}`, `{baseHost}`, `{basePort}`, `{basePath}`, `{registrationId}` and `{action}` in the value, and it expands them for each request. `{basePort}` and `{basePath}` expand with their own `:` and `/`.

The value does not decide which chain serves the callback. A scoped (per-tenant) chain mounts the redirection endpoint for the same value that it validates, so the two uses agree. The unscoped chain mounts one endpoint, and it derives that endpoint from the flat `camunda.security.authentication.oidc.redirect-uri` only. CSL still sends a `redirect-uri` that only `providers.oidc.<id>` sets to the IdP, but the unscoped chain does not listen on that path. A flat block with a `client-id` adds a registration of its own, and CSL then sends and serves the flat value. One exception applies: a provider entry that reuses the flat `registration-id` [replaces the flat entry](#combining-the-flat-and-providers-shapes). That entry then gives the value CSL sends, while the flat value still decides the served path, so such an entry must repeat the flat `redirect-uri`. A flat block that sets only `redirect-uri` moves the served path and changes no value a provider sends. An unscoped deployment on the providers shape must therefore set a custom callback path in two places: in the flat block, which moves the mounted endpoint, and in each provider entry, which gives the value for the IdP. Routing is also a property of the host: a chain sees the callback only if its own `securityMatcher` includes that path. For the webapp chain, `SecurityPathPort.webappPaths()` gives that matcher, so a host that moves the callback off the webapp paths must add the new path there.

Two callers derive a redirection endpoint from the value: the webapp client beans, and the webapp chains. Such a caller rejects the value at startup unless the value expands, under the `server.servlet.context-path` of the deployment, to all of these:

- an absolute `http`/`https` URL with a host, a path, and a port in 1–65535 if it has a port;
- a URL without a fragment, because RFC 6749 §3.1.2 does not permit a fragment in a redirection endpoint URI (a query is permitted, and CSL sends it to the IdP);
- a path that the default `StrictHttpFirewall` of Spring Security permits: the firewall rejects an empty segment (`//`), a dot segment (`/./`, `/../`), a semicolon, a backslash, a percent escape of one of these, and a control character, and it rejects such a request before a chain examines it;
- a callback path that the redirection endpoint derived from the same value serves.

A rejection in the provider map names the provider and both properties that can set the value. A flat `redirect-uri` that adds no registration of its own names only `camunda.security.authentication.oidc.redirect-uri`, because there is no provider to name. Two callers derive no browser login route: the token validation of an API chain, and UserInfo augmentation. Neither redirects a browser, and neither mounts a redirection endpoint, so neither makes this check. Both also do not make the registration-id check below, because they use the id as a registration key and never resolve `/oauth2/authorization/<id>`. Neither checks `end-session-endpoint-uri`, because the webapp logout handler is its only consumer. An API-only deployment therefore continues to start with a `redirect-uri` it cannot serve, or with an id the login route cannot address, and the login paths continue to reject both values.

The same startup pass rejects each provider block that Spring itself cannot make a `ClientRegistration` from: a blank `client-authentication-method`; a `scope` entry that holds a list separated by spaces instead of one value per entry; an incorrect value in a configured endpoint URL (`issuer-uri`, `authorization-uri`, `token-uri`, `jwk-set-uri`, `additional-jwk-set-uris`, `end-session-endpoint-uri`, and `user-info-uri` unless `user-info-enabled: false` discards it; `end-session-endpoint-uri` for a caller that mounts a login chain), which is a value that is not an absolute `http`/`https` URL with a host, or that has a port outside 1–65535; and a registration id that is not addressable as one path segment (the login route is `<basePath>/oauth2/authorization/<id>`). The failure only moves from the first login attempt to startup, where the configuration is.

A block must also be complete enough to build from, and what counts as complete depends on the same two kinds of caller as above. A caller that mounts the browser login chain (the webapp client beans, the webapp chains) needs a non-blank `client-id`, and either `issuer-uri` or all three of `authorization-uri`, `token-uri` and `jwk-set-uri` — the login flow needs both endpoints, and Spring's own `ClientRegistration.Builder` needs a client to authenticate with. A caller that derives no login route needs neither `client-id` nor `authorization-uri`/`token-uri` at this startup pass: only `issuer-uri` or `jwk-set-uri` is required here. A **single-provider** scope written for API-only token validation — no UserInfo augmentation enabled — can therefore set just `jwk-set-uri` (or `issuer-uri`) and nothing else. See [ADR-0027](../adr/0027-relax-endpoint-completeness-for-token-decoding-only-providers.md).

This `jwk-set-uri`-alone allowance is narrower than it looks, in two ways:

- **It does not extend to UserInfo augmentation.** When `user-info-augmentation.enabled: true`, at least one provider in the scope must additionally set `issuer-uri` with UserInfo left enabled for it (`user-info-enabled: true`, the default) — augmentation resolves the UserInfo endpoint from the issuer's discovery document, and `jwk-set-uri` carries no such endpoint. A scope that enables augmentation with only `jwk-set-uri`-configured providers fails at startup with an actionable "no issuer→userInfoUri mapping" error rather than running unaugmented; see [UserInfo claim augmentation](#userinfo-claim-augmentation) below.
- **It does not extend to a multi-provider scope**, augmentation or not. `ScopedJwtDecoderFactory` selects an issuer-aware decoder as soon as a scope configures two or more providers, and that decoder routes an incoming token to its provider by the token's `iss` claim — so every provider in such a scope needs `issuer-uri`, `jwk-set-uri` alone is not enough for any of them. A scope with two or more providers that leaves one without `issuer-uri` fails at startup with `The following OIDC Providers are missing 'issuerUri': <ids>`, naming every provider still missing one. The `jwk-set-uri`-alone shape above is therefore only reachable for a scope with exactly one OIDC provider.

Values that look correct and are rejected:

| Value | Why |
|-------|-----|
| `{baseUrl}api/callback` | no `/` after the placeholder, so the value expands to a different host (`https://hostapi/callback`) |
| `https://example.com/{basePath}/sso-callback` | `{basePath}` is in a segment of its own: it is empty on a root deployment, and it is an additional segment under a context path |
| `https://example.com:{basePort}/sso-callback` | `{basePort}` brings its own `:`, so a port that is not the default expands to `example.com::8443` |
| `https://example.com/orchestration` under `server.servlet.context-path=/orchestration` | the complete path is the context path, so no callback segment stays for the redirection endpoint |
| `https://example.com:99999/sso-callback` | URI syntax accepts the port, but no component can open a TCP connection to it |
...
> **Note:** augmentation sources the UserInfo endpoint URI from the `ClientRegistration`. Setting `user-info-enabled: false` nulls that URI. If that leaves *every* configured provider without a UserInfo URI (a single-IdP setup, or a multi-IdP setup where all providers disable it), CSL fails fast at startup with an `IllegalStateException` rather than silently running without augmentation — enabling augmentation with nothing it can ever augment is treated as a configuration mismatch. That check reads the configuration only; a provider whose resolved metadata exposes no UserInfo URI fails the claims lookups of its own issuer instead. If you supply your own `ClientRegistrationRepository`, CSL cannot read the configuration behind it, so the same mismatch reaches the first claims lookup that needs augmentation, as an `AuthenticationServiceException` (HTTP 500) with the `IllegalStateException` as its cause. In a multi-IdP setup where only *some* providers disable `user-info-enabled`, the coupling is silent instead: tokens from those providers' issuers skip augmentation at request time with no error surfaced (logged at DEBUG). Leave `user-info-enabled` at its default (`true`) for any provider you want augmentation to cover. See [ADR-0007](../adr/0007-oidc-user-info-enabled-toggle.md) for the full mechanism, and [ADR-0025](../adr/0025-deferred-oidc-resolution.md) for the scope of a resolution failure.
...
The callback path has two sides, and a custom path must be set on both sides. The unscoped webapp chain mounts its single redirection endpoint from the flat `camunda.security.authentication.oidc.redirect-uri`. The value CSL sends to the IdP comes from the `redirect-uri` of each provider entry. The examples here set neither value, so the callback stays at the default `/sso-callback` for each provider, which is the path the chain mounts. To serve the conventional path of Spring instead, set `redirect-uri: "{baseUrl}/login/oauth2/code/{registrationId}"` on the flat block **and** on each provider entry. The flat value moves the mounted endpoint, and the `{registrationId}` in it becomes a wildcard for one segment, so one endpoint serves each registration. The provider values put the related callback in the authorization request. If you set the value in one of the two places only, CSL sends the browser to a path nothing listens on, or a chain listens on a path CSL never sends to the IdP. See [`redirect-uri` and the redirection endpoint](#redirect-uri-and-the-redirection-endpoint). A flat `redirect-uri` needs no `client-id` next to it if the flat block must add no provider of its own.
...
public CamundaAuthenticationConverter<Authentication> authenticationConverter(
  ...
  return new OidcTokenAuthenticationConverter(
      lazyTokenClaimsConverter, claimsProvider, tokenClaimsConvertersByIssuer.getIfAvailable());
}
```
...
- The flat block only becomes a registration once it names a client identity or an endpoint to build one from — `client-id`, `issuer-uri`, `authorization-uri`, `token-uri`, or `jwk-set-uri` (so a token-decoding-only block's bare `jwk-set-uri` still counts; see [ADR-0027](../adr/0027-relax-endpoint-completeness-for-token-decoding-only-providers.md)). Setting only `redirect-uri` or `post-logout-redirect-uri` on the flat block — the shape used to move the callback path while `providers.oidc.*` entries supply the actual registrations, see [`redirect-uri` and the redirection endpoint](#redirect-uri-and-the-redirection-endpoint) above — contributes no registration of its own, so `post-logout-redirect-uri` set there alone has no effect either: there is no registration for it to apply to, and it only ever does anything for a registration that also mounts the login chain.
