...
A block must also be complete enough to build from, and what counts as complete depends on the same two kinds of caller as above. A caller that mounts the browser login chain (the webapp client beans, the webapp chains) needs a non-blank `client-id`, and either `issuer-uri` or all three of `authorization-uri`, `token-uri` and `jwk-set-uri` — the login flow needs both endpoints, and Spring's own `ClientRegistration.Builder` needs a client to authenticate with. A caller that derives no login route (the token validation of an API chain, UserInfo augmentation) needs neither `client-id` nor `authorization-uri`/`token-uri`: only `issuer-uri` or `jwk-set-uri` is required, since that is the only property token decoding reads. A provider block written for token decoding only can therefore set just `jwk-set-uri` (or `issuer-uri`) and nothing else. See [ADR-0027](../adr/0027-relax-endpoint-completeness-for-token-decoding-only-providers.md).
