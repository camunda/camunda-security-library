...
- an absolute `http`/`https` URL with a host, a path, and — where it names a port — one in the range 0–65535, since `java.net.URI` parses any run of digits but no socket can be opened on `:99999`;
...
The same startup pass rejects any provider block Spring itself could not turn into a `ClientRegistration`: a blank `client-id` or `client-authentication-method`, a `scope` entry holding a space-separated list rather than one value per entry, a malformed value — not an absolute `http`/`https` URL with a usable host and port — in any configured endpoint URL (`issuer-uri`, `authorization-uri`, `token-uri`, `jwk-set-uri`, `additional-jwk-set-uris`, `end-session-endpoint-uri`, and `user-info-uri` unless `user-info-enabled: false` discards it), and a registration id that is not addressable as a single path segment (the login route is `<basePath>/oauth2/authorization/<id>`). None of these values became newly mandatory — the failure simply moves from the first login attempt to startup, where the configuration is.
...
| `https://example.com:99999/sso-callback` | a port no socket can be opened on, which `java.net.URI` and `java.net.URL` both accept |
