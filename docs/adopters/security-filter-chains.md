...
- The flat block only becomes a registration once at least one of its properties is set (`client-id` included, but also, for example, a token-decoding-only block's `jwk-set-uri`; see [ADR-0027](../adr/0027-relax-endpoint-completeness-for-token-decoding-only-providers.md)). Setting `post-logout-redirect-uri` on an otherwise-untouched flat block has no effect, because there is no registration for it to apply to — and a `post-logout-redirect-uri` only ever does anything for a registration that also mounts the login chain in the first place.
