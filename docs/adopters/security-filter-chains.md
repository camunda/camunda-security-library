...
For the rationale behind this design — why the chains live in CSL, and why hosts opt in via explicit `@Import` rather than relying on Spring Boot auto-configuration — see [ADR-0006](../adr/0006-no-spring-boot-auto-configuration.md).
...
3. `@Import` the configuration classes you want active. The CSL does not use Spring Boot auto-configuration ([ADR-0006](../adr/0006-no-spring-boot-auto-configuration.md)) — nothing activates by simply adding the dependency. Hosts opt in to each capability explicitly:
...
| `additional-jwk-set-uris` | list&lt;string&gt; | empty | Secondary JWK Set URIs consulted when the primary `jwk-set-uri` does not resolve a token's signing key. See [Multiple JWK Set URIs](#multiple-jwk-set-uris) below and [ADR-0009](../adr/0009-multi-idp-oidc-configuration.md). |
| `authorization-uri`, `token-uri`, `user-info-uri` | string | unset | Endpoint overrides for non-discovery flows. |
| `user-info-enabled` | boolean | `true` | When `false`, the built `ClientRegistration` has its `userInfoUri` nulled so Spring Security does not call the IdP's UserInfo endpoint after token exchange. See [Disabling the UserInfo fetch](#disabling-the-userinfo-fetch) below and [ADR-0010](../adr/0010-oidc-user-info-enabled-toggle.md). |
...
> [ADR-0010](../adr/0010-oidc-user-info-enabled-toggle.md) for the full
> mechanism.
...
See [ADR-0009](../adr/0009-multi-idp-oidc-configuration.md) for the design rationale.
...
See [ADR-0009](../adr/0009-multi-idp-oidc-configuration.md) for the design rationale, the choice of composite `JWKSource` over Spring's `JwtIssuerAuthenticationManagerResolver`, and the lazy failure model.
...
A host-supplied `OidcUserService` bean still takes precedence in both modes (see [ADR-0010](../adr/0010-oidc-user-info-enabled-toggle.md)) — `user-info-enabled` only governs the library's default wiring.
...
[ADR-0019](../adr/0019-cors-and-https-redirect-host-hooks.md) for why this is a method
...
See [ADR-0019](../adr/0019-cors-and-https-redirect-host-hooks.md) for the design rationale.
...
The actual permission decision delegates to the unified `AuthorizationCheckPort` (ADR-0017): the filter asks for `ACCESS` on the resolved web app as a `COMPONENT` resource and treats `Either.right(...)` as authorized. Hosts supply an `AuthorizationCheckPort` — either their own bean, or the ingredients for the library default (an `AuthorizationScopeRepositoryPort`, from which `AuthorizationConfiguration` builds `AuthorizationService`). Activation rationale lives in [ADR-0017](../adr/0017-unified-authz-framework-in-core.md).
...
A third concern — which paths bypass the check entirely (typically the setup endpoint plus its static assets) — is declared on the existing `SecurityPathPort` via `adminFilterBypassPaths()`, alongside the host's other path declarations. Activation rationale lives in [ADR-0007](../adr/0007-admin-user-setup-spis.md).
...
1. Replace your `@Bean SecurityFilterChain` methods by deleting them. Add an `@Import` list for the library's configuration classes — pick the ones that match your auth method and API protection mode (see the [Quickstart](#quickstart) snippet). Per [ADR-0006](../adr/0006-no-spring-boot-auto-configuration.md) hosts opt in to each capability explicitly; nothing activates by simply adding the dependency.
