...
The same startup pass rejects each provider block that Spring itself cannot make a `ClientRegistration` from: a blank `client-id` or `client-authentication-method`; a `scope` entry that holds a list separated by spaces instead of one value per entry; an incorrect value in a configured endpoint URL (`issuer-uri`, `authorization-uri`, `token-uri`, `jwk-set-uri`, `additional-jwk-set-uris`, `end-session-endpoint-uri`, and `user-info-uri` unless `user-info-enabled: false` discards it; `end-session-endpoint-uri` for a caller that mounts a login chain), which is a value that is not an absolute `http`/`https` URL with a host, or that has a port outside 1–65535; and a registration id that is not addressable as one path segment (the login route is `<basePath>/oauth2/authorization/<id>`). No value becomes mandatory. The failure only moves from the first login attempt to startup, where the configuration is.
