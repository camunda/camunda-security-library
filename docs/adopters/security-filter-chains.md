...
- **A resolvable `jwk-set-uri` is required.** Set `jwk-set-uri` explicitly, or set `issuer-uri` so OIDC discovery populates it. A provider that resolves neither fails with `OIDC Provider '<id>' missing valid 'jwk-set-uri'` at the first token decode. The decoder is built on first use, so an identity provider CSL cannot reach at startup does not stop the application context.
