...
This workflow creates GitHub bug issues that are **self-contained enough for a fresh agent session to pick up and deliver a complete fix without further conversation**.

It applies when a user says "track this bug", "this is broken", "the behavior should be X", or describes unexpected behavior they want fixed.

Claude Code users: this workflow is also available as `/bug`.
